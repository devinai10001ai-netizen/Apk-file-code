...
const parseParams = (params) => {
  try {
    return {
      ...
      gradient: params.gradient ? JSON.parse(params.gradient) : ['#FF6B35', '#FF5722'],
      title: params.title || 'Insurance',
    };
  } catch (e) {
    return {
      type: 'category',
...
// ============== HERO SECTION ==============
const HeroSection = ({ data, gradient, title, type }) => {
  const router = useRouter();
  
  return (
    <LinearGradient colors={gradient} style={heroStyles.container}>
      <SafeAreaView edges={['top']}>
        <View style={heroStyles.header}>
          <TouchableOpacity onPress={() => router.back()} style={heroStyles.backBtn}>
            <Ionicons name="arrow-back" size={22} color="#fff" />
          </TouchableOpacity>
          <Text style={heroStyles.headerTitle} numberOfLines={1}>{title}</Text>
        ...
        <View style={heroStyles.heroContent}>
          ...
          <Text style={heroStyles.title}>{data.name || title}</Text>
          <Text style={heroStyles.description}>{data.description || 'Comprehensive coverage for you and your family'}</Text>
        ...
        <View style={heroStyles.statsRow}>
          ...
          {!data.coverage && !data.premium && !data.claimRatio && (
            <>
              <View style={heroStyles.statItem}>
                <Text style={heroStyles.statValue}>10000+</Text>
                <Text style={heroStyles.statLabel}>Hospitals</Text>
              </View>
              <View style={heroStyles.statItem}>
                <Text style={heroStyles.statValue}>85%+</Text>
                <Text style={heroStyles.statLabel}>Claims</Text>
              </View>
              <View style={heroStyles.statItem}>
                <Text style={heroStyles.statValue}>24/7</Text>
                <Text style={heroStyles.statLabel}>Support</Text>
              </View>
            </>
          )}
        </View>
...
const CoverageSection = ({ coverages, title = 'Choose Coverage' }) => {
  const [selected, setSelected] = useState(coverages?.[0] || '5L');
  
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>{title}</Text>
      <Text style={sectionStyles.sectionSubtitle}>Select the sum insured that best fits your needs</Text>
      <View style={coverageStyles.chips}>
        {(coverages || ['5L', '10L', '25L', '50L']).map((cov, index) => (
          <TouchableOpacity 
            key={index} 
...
const FeaturesSection = ({ features, title = 'Key Features' }) => {
  const items = features || ['Comprehensive coverage', 'Cashless hospitalization', 'Tax benefits', '24/7 Support'];
...
const PremiumCalculatorSection = () => {
  ...
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>Premium Calculator</Text>
      <Text style={sectionStyles.sectionSubtitle}>Calculate your estimated premium</Text>
...
const InsurersSection = ({ insurers }) => {
  ...
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>Top Insurers</Text>
      <Text style={sectionStyles.sectionSubtitle}>Compare quotes from leading insurers</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
        {items.map((insurer, index) => (
...
const BenefitsBreakdownSection = ({ data }) => {
  ...
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>Benefits Breakdown</Text>
...
const ClaimProcessSection = () => {
  ...
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>Claim Process</Text>
      <Text style={sectionStyles.sectionSubtitle}>Simple 4-step claim process</Text>
...
const FAQSection = () => {
  ...
  const faqs = [
    ...
    { question: 'What are the tax benefits?', answer: 'You can claim tax deduction up to Rs 1.5L under Section 80D for premium paid.' },
  ];
  
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>Frequently Asked Questions</Text>
      {faqs.map((faq, index) => (
        <TouchableOpacity 
          key={index} 
    ...
    <View style={ctaStyles.container}>
      <LinearGradient colors={['#FF6B35', '#FF5722']} style={ctaStyles.gradient}>
        ...
        <Text style={ctaStyles.subtitle}>
          {type === 'offer' 
            ? `Use code ${data?.code || 'SAVE20'} to get ${data?.discount || '20%'} off`
...
const ExpertSection = () => {
  ...
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>Talk to Our Experts</Text>
      <Text style={sectionStyles.sectionSubtitle}>Get personalized advice from our insurance experts</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
...
const ReviewsSection = () => {
  ...
  return (
    <View style={sectionStyles.container}>
      <Text style={sectionStyles.sectionTitle}>Customer Reviews</Text>
      <Text style={sectionStyles.sectionSubtitle}>See what our customers say</Text>
...
export default function DetailScreen() {
  ...
  const { type, data, gradient, title } = parseParams(params);
        ...
        return (
          <>
            <View style={offerStyles.detailCard}>
              ...
              <Text style={offerStyles.detailDesc}>{data?.description || 'Special discount offer for limited time'}</Text>
              <View style={offerStyles.codeRow}>
                <Text style={offerStyles.codeLabel}>Use Code:</Text>
                <View style={offerStyles.codeBox}>
                  <Text style={offerStyles.codeText}>{data?.code || 'SAVE20'}</Text>
                </View>
              </View>
              <Text style={offerStyles.validity}>Valid until: {data?.validTill || '2026-12-31'}</Text>
            </View>
        ...
        return (
          <>
            <View style={expertDetailStyles.profile}>
              ...
              <Text style={expertDetailStyles.name}>{data?.name || 'Expert'}</Text>
              <Text style={expertDetailStyles.specialty}>{data?.specialty || 'Insurance Expert'}</Text>
              <View style={expertDetailStyles.stats}>
                <View style={expertDetailStyles.stat}>
                  <Ionicons name="star" size={18} color="#FFD700" />
                  <Text style={expertDetailStyles.statValue}>{data?.rating || '4.5'}</Text>
                  <Text style={expertDetailStyles.statLabel}>Rating</Text>
                </View>
                <View style={expertDetailStyles.stat}>
                  <Ionicons name="people" size={18} color="#3498DB" />
                  <Text style={expertDetailStyles.statValue}>{data?.reviews || '500'}</Text>
                  <Text style={expertDetailStyles.statLabel}>Reviews</Text>
                </View>
                <View style={expertDetailStyles.stat}>
                  <Ionicons name="time" size={18} color="#9B59B6" />
                  <Text style={expertDetailStyles.statValue}>{data?.experience || '5 years'}</Text>
  ...
  return (
    <ScrollView style={mainStyles.container} showsVerticalScrollIndicator={false}>
      <HeroSection data={data} gradient={gradient} title={title} type={type} />
      {renderContent()}
    </ScrollView>
  );
}
